...
export class WorkspacesService {
  ...
  async createWorkspace(userId: string, email: string, createWorkspaceDto: CreateWorkspaceDto) {
  ...
  async addMember(userId: string, workspaceId: string, email: string, role: 'ADMIN' | 'MEMBER') {
  ...
  async updateWorkspace(userId: string, workspaceId: string, data: { name: string }) {
    ...
    if (!membership || membership.role !== 'OWNER') {
      throw new ForbiddenException('Only the owner can update the workspace name');
    }
