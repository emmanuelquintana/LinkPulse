...
export default function DashboardHomePage() {
  ...
  const totalClicksCount = links.reduce((acc, curr) => acc + curr.clicksCount, 0);
  
  // Fake calculation for CTR since we don't have impressions track yet
  const avgCtr = activeLinksCount > 0 ? (totalClicksCount / (activeLinksCount * 100) * 100).toFixed(1) : "0.0";
    ...
    <div className="max-w-[1400px] mx-auto space-y-6 font-sans">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm flex flex-col justify-between">
          <div className="flex items-start justify-between">
            <span className="text-sm font-semibold text-gray-500">Total Clicks</span>
            <div className="h-10 w-10 rounded-lg bg-indigo-50 flex items-center justify-center">
              <span className="material-symbols-outlined text-indigo-600">touch_app</span>
            </div>
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mt-2 mb-4 tracking-tight">{totalClicksCount.toLocaleString()}</h2>
          <div className="flex items-center text-sm">
            <span className="flex items-center font-bold text-emerald-600">
              <span className="material-symbols-outlined text-[16px] mr-1">trending_up</span> --%
            </span>
            <span className="font-medium text-gray-400 ml-1.5">vs last 7 days</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm flex flex-col justify-between">
          <div className="flex items-start justify-between">
            <span className="text-sm font-semibold text-gray-500">Active Links</span>
            <div className="h-10 w-10 rounded-lg bg-emerald-50 flex items-center justify-center">
              <span className="material-symbols-outlined text-emerald-600">link</span>
            </div>
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mt-2 mb-4 tracking-tight">{activeLinksCount.toLocaleString()}</h2>
          <div className="flex items-center text-sm">
            <span className="flex items-center font-bold text-emerald-600">
               <span className="material-symbols-outlined text-[16px] mr-1">trending_up</span> --%
            </span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm flex flex-col justify-between">
          <div className="flex items-start justify-between">
            <span className="text-sm font-semibold text-gray-500">Avg. CTR</span>
            <div className="h-10 w-10 rounded-lg bg-amber-50 flex items-center justify-center">
               <span className="material-symbols-outlined text-amber-500">ads_click</span>
            </div>
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mt-2 mb-4 tracking-tight">{avgCtr}%</h2>
          <div className="flex items-center text-sm">
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-8 flex flex-col relative">
        <div className="flex items-center justify-between mb-10">
          <div>
            <h3 className="text-xl font-black text-gray-900 tracking-tight">Clicks Overview</h3>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Analytics performance</p>
          </div>
          ...
          <div className="relative">
            ...
            >
              <span className="material-symbols-outlined text-[20px] text-gray-400 group-hover:text-indigo-600">calendar_today</span>
              Last {selectedDays} days
              <span className="material-symbols-outlined text-[18px] text-gray-400 group-hover:text-indigo-600 transition-transform duration-200" style={{ transform: isDropdownOpen ? 'rotate(180deg)' : 'none' }}>expand_more</span>
            </button>
            ...
            {isDropdownOpen && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-2xl shadow-2xl p-1.5 z-20 animate-in zoom-in-95 duration-150 origin-top-right ring-1 ring-black/5">
                  <button 
                    onClick={() => { setSelectedDays(7); setIsDropdownOpen(false); }}
                  ...
                  >
                    Last 7 days
                    {selectedDays === 7 && <span className="material-symbols-outlined text-[18px]">check</span>}
                  </button>
                  <button 
                    onClick={() => { setSelectedDays(30); setIsDropdownOpen(false); }}
                  ...
                  >
                    Last 30 days
                    {selectedDays === 30 && <span className="material-symbols-outlined text-[18px]">check</span>}
                  </button>
                </div>
              </>
            )}
        ...
        {links.length === 0 ? (
          <div className="w-full h-[320px] flex flex-col items-center justify-center border-t border-gray-50">
             ...
             <p className="text-sm text-gray-400 font-medium">Create and share a link to start seeing statistics.</p>
          </div>
        ) : (
          <div className="w-full h-[320px] flex items-end justify-between relative">
            ...
            <div className="flex-1 ml-14 h-full relative group">
               ...
               <div className="absolute inset-x-0 bottom-10 top-0 z-10 opacity-90 overflow-hidden px-1">
                  ...
                  <svg preserveAspectRatio="none" className="w-full h-full" viewBox={`0 0 ${svgWidth} ${svgHeight}`}>
                    <defs>
                    ...
                    <path fill="url(#chartFillGradient)" d={fillPathD} className="transition-all duration-700" />
                    <path 
                      fill="none" 
                      stroke="#4f46e5" 
                      strokeWidth="4" 
               ...
               <div className="absolute bottom-0 inset-x-0 h-10 flex justify-between items-center text-[0.65rem] font-black text-gray-400 px-1">
                  {dayNames.map((name, i) => {
                    ...
                    if (selectedDays === 30) {
                       if (i % 5 !== 0 && i !== dayNames.length - 1) return <div key={i} className="w-0 overflow-visible opacity-0"></div>;
                    }
                    return <span key={i} className="uppercase tracking-widest">{name}</span>;
                  })}
               </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-visible relative">
        <div className="flex items-center justify-between p-8 border-b border-gray-50">
          <div>
            <h3 className="text-xl font-black text-gray-900 tracking-tight">Recent Links</h3>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Latest activity</p>
          </div>
          <Link href="/dashboard/links" className="flex items-center gap-1.5 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-black hover:bg-indigo-600 hover:text-white transition-all group">
            View All Links
          ...
          <table className="w-full text-sm text-left border-separate border-spacing-y-1">
            ...
            <tbody>
              ...
              {recentLinks.length > 0 ? recentLinks.map((link) => (
                <tr key={link.id} className="group hover:bg-gray-50/80 transition-all rounded-2xl">
                  <td className="px-6 py-5 first:rounded-l-2xl">
                     <div className="flex flex-col">
                       <span className="font-black text-gray-900 tracking-tight">localhost:3002/{link.customAlias || link.shortCode}</span>
                       <span className="text-[0.65rem] text-gray-400 font-bold uppercase tracking-widest mt-0.5 truncate max-w-[150px]">{link.title || 'Untitled'}</span>
                     </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex justify-center">
                      <div className="flex items-center gap-2 text-gray-500 font-bold text-xs bg-white px-3 py-2 rounded-xl border border-gray-100 shadow-sm max-w-[200px] group-hover:border-indigo-100/50 transition-colors" title={link.originalUrl}>
                        <span className="material-symbols-outlined text-[16px] text-gray-300">link</span>
                        <span className="truncate">{link.originalUrl.replace(/^https?:\/\//, '')}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td colSpan={5} className="px-6 py-20 text-center">
                     <div className="flex flex-col items-center gap-4">
                       <div className="h-16 w-16 rounded-full bg-gray-50 flex items-center justify-center">
                         <span className="material-symbols-outlined text-4xl text-gray-200">link_off</span>
                       </div>
                       <p className="text-gray-900 font-black">No active links found</p>
                       <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">Share some links to see activity here</p>
                     </div>
                  </td>
                </tr>
              )}
