...
export default function WorkspacesPage() {
  ...
  const loadWorkspaces = async () => {
    ...
    try {
      ...
      else if (data && data.data && Array.isArray(data.data)) setWorkspaces(data.data);
    } catch (err) {
  ...
  const handleDelete = async (wsId: string) => {
    if (!confirm('Are you sure you want to delete this workspace? All associated links and data will be permanently removed.')) return;
    ...
    <>
      <div className="max-w-[1400px] mx-auto font-sans animate-in fade-in duration-500">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Workspaces</h2>
            <p className="text-gray-500 font-medium mt-1">Manage your teams and collaboration environments.</p>
          </div>
          <button 
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-visible">
            {workspaces.map((ws) => (
              <div key={ws.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm p-7 hover:border-indigo-200 transition-all group relative">
                ...
                <h3 className="text-xl font-black text-gray-900 truncate pr-6 mb-1">{ws.name}</h3>
                <p className="text-[0.65rem] text-gray-400 font-bold uppercase tracking-[0.1em]">Free Workspace</p>
                
                <div className="grid grid-cols-2 gap-4 mt-8 pt-6 border-t border-gray-50">
                  <div className="flex flex-col">
                    <span className="text-[0.65rem] font-bold text-gray-400 uppercase tracking-widest mb-1">Members</span>
                    <span className="text-2xl font-black text-gray-900">{ws._count?.members || 1}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[0.65rem] font-bold text-gray-400 uppercase tracking-widest mb-1">Links</span>
                    <span className="text-2xl font-black text-gray-900">{ws._count?.links || 0}</span>
                  </div>
                </div>
                ...
                <div className="mt-8 flex items-center justify-between">
                  ...
                  <div className="flex -space-x-2">
                     {[1, 2].map(i => (
                       <div key={i} className="h-8 w-8 rounded-full border-2 border-white bg-indigo-100 flex items-center justify-center text-[10px] font-black text-indigo-600 ring-1 ring-gray-100">
                          U{i}
                       </div>
                     ))}
                  </div>
            ...
            {workspaces.length === 0 && !loading && (
              <div className="col-span-full py-24 bg-white rounded-[2rem] border-2 border-dashed border-gray-100 flex flex-col items-center justify-center text-center">
                 <div className="h-20 w-20 rounded-full bg-gray-50 flex items-center justify-center mb-6">
                   <span className="material-symbols-outlined text-4xl text-gray-300">group_work</span>
                 </div>
                 <h3 className="text-2xl font-black text-gray-900">No workspaces yet</h3>
                 <p className="text-gray-500 max-w-xs mt-2 font-medium">Workspaces allow you to organize your links and collaborate with your team.</p>
                 <button 
                  onClick={() => setShowCreateModal(true)}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden ring-1 ring-gray-200 animate-in zoom-in-95 duration-200">
              <div className="flex justify-between items-center p-6 border-b border-gray-100">
                <h3 className="text-xl font-black text-gray-900 tracking-tight">Create Workspace</h3>
                <button onClick={() => setShowCreateModal(false)} className="text-gray-400 hover:text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-full p-1 transition-colors">
              ...
              <form onSubmit={handleCreateWorkspace} className="p-7 space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[0.7rem] font-black text-gray-400 uppercase tracking-widest block px-1">Workspace Name</label>
                  <input 
                    type="text" 
                    required 
